...
class Simulator(object):
    ...
    def __init__(self, env, size=None, update_delay=1.0, display=True):
        ...
        self.update_delay = update_delay  # duration between each step (in
        # secs)
        ...
        if self.display:
            try:
                ...
                self.frame_delay = max(1, int(
                    self.update_delay * 1000))  # delay between GUI frames
                # in ms (min: 1)
                self.agent_sprite_size = (32, 32)
                self.agent_circle_radius = 10  # radius of circle,
        ...
        for trial in xrange(n_trials):
            ...
            while True:
                try:
                    ...
                    # print "Simulator.run(): current_time = {:.3f}".format(
                    # self.current_time)
    ...
    def render(self):
        ...
        for agent, state in self.env.agent_states.iteritems():
            # Compute precise agent location here (back from the
            # intersection some)
            agent_offset = (
                2 * state['heading'][0] * self.agent_circle_radius,
                2 * state['heading'][1] * self.agent_circle_radius)
            agent_pos = (
            state['location'][0] * self.env.block_size - agent_offset[0],
            ...
            if state['destination'] is not None:
                self.pygame.draw.circle(self.screen, agent_color, (
        ...
        for text in self.env.status_text.split('\n'):
            self.screen.blit(self.font.render(text, True, self.colors['red'],
